      ...
      <div className="hidden h-full w-full items-center justify-center p-6 md:flex">
        <div className="relative w-full max-w-md overflow-hidden rounded-3xl border border-neutral-800 bg-neutral-950/95 shadow-2xl backdrop-blur">
          <div className="flex items-center justify-between border-b border-neutral-800 px-5 py-4">
            <h2 className="text-sm font-bold text-white">Likes</h2>
                      ...
                      {!isMe &&
                        (isFollowed ? (
                          <button
                            ...
                            disabled={followLoading === user.username}
                            className="flex items-center gap-1 rounded-full border border-neutral-600 px-3 py-1.5 text-[11px] font-medium text-neutral-300 transition-colors hover:bg-neutral-800 disabled:opacity-50"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              className="h-3 w-3"
                          ...
                          <button
                            onClick={() => handleFollow(user.id, user.username)}
                            disabled={followLoading === user.username}
                            className="rounded-full bg-violet-600 px-3 py-1.5 text-[11px] font-semibold text-white transition-colors hover:bg-violet-500 disabled:opacity-50"
                          >
                            {followLoading === user.username ? "..." : "Follow"}
                          </button>
                        ))}
